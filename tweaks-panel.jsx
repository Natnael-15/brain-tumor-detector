// tweaks-panel.jsx – NeuroScan settings / tweaks panel (loaded by Brain Tumor Detector.html)
...
// ── TweaksPanel ───────────────────────────────────────────────────────────────
function TweaksPanel({ settings, onSettingsChange }) {
    ...
    >
      ...
      <Section title="Preprocessing" icon="⚙️">
        <Select
          ...
          onChange={(v) => update("normalization", v)}
          options={[
        ...
        <Slider
          ...
          onChange={(v) => update("gaussianBlur", v)}
        />
        <Toggle
          label="Bias Field Correction"
          checked={settings.biasCorrection}
          onChange={(v) => update("biasCorrection", v)}
        />
        <Toggle
      ...
      <Section title="Report" icon="📄">
        ...
        <Toggle
          ...
          onChange={(v) => update("includeProbs", v)}
        />
        <Toggle
          label="HIPAA-Anonymize DICOM Tags"
          checked={settings.anonymize}
          onChange={(v) => update("anonymize", v)}
        />
      </Section>
      ...
      <Section title="Performance" icon="⚡">
        ...
        <Toggle
          ...
          onChange={(v) => update("cacheScans", v)}
        />
      </Section>
    </div>
  );
...
// Export so the main HTML can use it (namespaced to avoid collisions)
window.NeuroScan = window.NeuroScan || {};
window.NeuroScan.TweaksPanel = TweaksPanel;
window.NeuroScan.defaultTweaksSettings = {
  model: "unet3d",
  precision: "fp32",
  tta: false,
  confidence: 0.5,
